...
public class SqlFilterProxyTest {
    ...
    public void test() {
        ...
        SqlTypeTestUtil.executeUpdate(conn, "delete from T_SCHEDULE");

        {
            val sql = "insert into T_SCHEDULE(id, name, schedule_state, subscribes) values(?, ?, '正常', 9)";
            @Cleanup val stmt = conn.prepareStatement(sql);
            SqlTypeTestUtil.executeUpdate(stmt, "1", "bingoo");
            SqlTypeTestUtil.executeUpdate(stmt, "2", "dingoo");
        }
        ...
        SqlTypeTestUtil.executeUpdate(conn, "update T_SCHEDULE set name = 'bingoohuang', schedule_state = '失效' where id = '1' and name = ?", "bingoo");
        {
            @Cleanup val stmt = conn.prepareStatement("update T_SCHEDULE set name = ? where id = ?");
            SqlTypeTestUtil.executeUpdate(stmt, "dingoohuang", "2");
        }
        ...
        List<Map<String, Object>> list = SqlTypeTestUtil.executeSelect(conn, "select id, name, schedule_state from T_SCHEDULE");
        ...
        SqlTypeTestUtil.executeUpdate(conn, "delete from T_SCHEDULE where id = '1'");
        {
            @Cleanup val stmt = conn.prepareStatement("delete from T_SCHEDULE where id = ?");
            SqlTypeTestUtil.executeUpdate(stmt, "2");
        }
